...
Open [Steam Radar](https://three-coins-studio.github.io/steam-radar/) and enter the shared team password supplied by a maintainer. The password unlocks the current browser tab only.

After unlocking, enter a Steam game name, choose YouTube, Twitch, or both, and select **Search**. Team members do not need an API key, Twitch login, Supabase project, or local installation.

## Architecture

GitHub Pages hosts the interface. The deployed Edge Function verifies the shared password against a private bcrypt hash in Postgres before doing any provider work. It then resolves the name with Steam Store search, enriches the selected app through SteamSpy, and queries YouTube and Twitch. Provider credentials and the password hash remain server-side and never reach the browser.
